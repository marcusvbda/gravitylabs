        ...
        <x-dropdown-menu class="ml-3" contentClass="max-h-[600px] w-[300px] overflow-y-auto">
            <x-slot name="source">
                <div wire:init="loadList"
                    ...
                    aria-current="page">
                    <div class="flex items-center gap-2 text-ellipsis max-w-32 overflow-hidden">
                    ...
                    @if (@$this->total)
                        <x-icons.arrow-down class="size-2" />
                    @endif
                </div>
            </x-slot>
            @if (@$this->total)
                    ...
                    <div wire:loading.remove>
                        @foreach ($this->items as $item)
                            <a href="#" wire:click.prevent="selectApp({{ $item->id }})"
                                class="flex px-4 py-2 text-sm text-gray-400 w-full overflow-hidden cursor-pointer dark:text-gray-500 items-center justify-between gap-2 hover:text-gray-900 dark:hover:text-white transition duration-300"
                                role="menuitem" tabindex="-1">
                                <div class="flex items-center gap-2 text-ellipsis max-w-28">
                                    <div class="size-3 min-w-3 min-h-3 rounded-full"
